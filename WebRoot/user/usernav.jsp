...
<%
	...
	if (null == userName) {
		request.setAttribute("Error",
				"请您先登录再继续操作！");
		response.sendRedirect("achieveIndex");
	}
...
<%
	...
	<div class="content" id="content">
      ...
      <!-- 得到用户的姓名 -->
	<!-- <s:iterator value="#request.list" id="singleuser"></s:iterator> -->
	<!-- 遍历单个用户的信息 -->

	welcome ${sessionScope.userName}!
    <s:a href="achieveSection.action?userID=%{#request.userID}&&userName=%{#request.userName}">首页</s:a>
	<s:a href="allPageSort.action?userID=%{#request.userID}&&userName=%{#request.userName}&&pageNow=1">显示所有帖子</s:a>
    <s:a href="pageSort.action?userID=%{#request.userID}&&userName=%{#request.userName}&&pageNow=1">我发表的帖子</s:a>
	<s:a href="publichSwitch.action?userID=%{#request.userID}&&userName=%{#request.userName}">发帖</s:a>
	<!-- 用户发帖 -->

	<s:a href="getSingleUser.action?userID=%{#singleuser.userID}">更改个人信息</s:a>

	<s:hidden name="userID" value="%{#singleuser.userID}"></s:hidden>
				...
				<div class="listhot">
					<ul style="margin:0;padding:0;">
						<s:iterator value="#request.hotList" id="hot">
							<li><s:a href="getSingleTopic.action?topicID=%{#hot.topicID}&&userID=%{#request.userID}&&userName=%{#request.userName}" width="150">
				...
				<div class="listhot">
					<ol style="margin:0;padding:0;">
						<s:iterator value="#request.newHotList" id="newhot">
							<li><s:a href="getSingleTopic.action?topicID=%{#newhot.topicID}&&userID=%{#request.userID}&&userName=%{#request.userName}">
									<s:property value="title" />
						...
						<s:iterator value="#request.listchildren" id="section1">
							<s:if test="parentSectionID==#section.sectionID">
								<tr height="50">
									...
									<td><s:a
											href="childrenPageSort.action?userID=%{#request.userID}&sectionID=%{#section1.sectionID}&pageNow=1">
